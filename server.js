...
// ==========================================
// 1. PROCESAR SPOTIFY (Con triple respaldo)
// ==========================================
async function procesarSpotify(input, res) {
    try {
        ...
        // INTENTO 2: SpotifyDown (CDN Respaldo)
        try {
            ...
            console.log('Error en CDN de respaldo SpotifyDown');
        }

        // INTENTO 3: FabDL (Servidor de Respaldo 2)
        try {
                ...
                if (fabData.result && fabData.result.gid) {
                    ...
                    if (convertRes.ok) {
                        ...
                        if (convertData.result && convertData.result.download_url) {
                            const rawAudioUrl = `https://api.fabdl.com${convertData.result.download_url}`;
                            const directDownloadProxyUrl = `/api/download-file?url=${encodeURIComponent(rawAudioUrl)}&name=${encodeURIComponent(titleCombined)}.mp3`;
            ...
            console.log('Error en FabDL de respaldo');
        }

        return res.status(400).json({
            exito: false,
            mensaje: 'No fue posible obtener el audio de Spotify. Por favor reintenta en unos segundos.'
        });
